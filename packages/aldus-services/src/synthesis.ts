 ...
 * silently retrying paid requests, and `@aldus/tts-ledger` deliberately has no way to make a
...
import type { ActorRef, Money } from "@aldus/core";
import type { GateEngine, SpendGrant } from "@aldus/gate-engine";
import type {
  AsrFinding,
...
import { planSubjectDigests } from "@aldus/tts-ledger";
...
export class SynthesisGateway {
  ...
  async synthesise(input: {
    ...
    // pattern `@aldus/release` uses for operation criticality. It makes the type unnameable
