 ...
 *
 * `@aldus-runtime/tts-ledger` ships only in-memory stores, because §7 requires core models to be
 ...
 *
 * §7 names exactly four per-run collection files, and `RunCollectionName` in `@aldus-runtime/file-store`
 ...
 * `@aldus-runtime/artifact-registry` set with `.aldus/artifacts/`: §7 calls its layout "recommended", and
 ...
 * `@aldus-runtime/file-store`'s `appendToCollection` uses, and for the same reason.
 */
