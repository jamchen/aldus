...
- `@aldus-runtime/artifact-registry` needs no adapter at all. Wiring it is plumbing.
- `@aldus-runtime/release` needs an adapter that talks to a destination, and an authority decision that
  `@aldus-runtime/gate-engine` already owns.
- `@aldus-runtime/tts-ledger` needs an adapter that spends real money, guarded by §13.2 — the sharpest
...
Authority comes from `@aldus-runtime/gate-engine` and is never re-decided. The executor is built **per
...
**There is no way to ask the services to skip reconciliation.** `@aldus-runtime/release` exposes a
...
`AldusError`s from `@aldus-runtime/release`, and both are mapped to `refused` rather than propagated.
...
- The permit handed to the adapter is branded with a `unique symbol` that is declared and never
  ...
  present only in the type — following the pattern `@aldus-runtime/release` uses for operation
...
`@aldus-runtime/tts-ledger` ships only in-memory stores, correctly: §7 keeps core models independent of
...
`@aldus-runtime/artifact-registry` already set the precedent with `.aldus/artifacts/`. §7 calls its layout
"recommended".
...
- `@aldus-runtime/services` now depends on seven workspace packages and is the widest in the repository.
...
- **Surface `reconcile: false` for parity with `@aldus-runtime/release`.** Rejected: parity is not a
